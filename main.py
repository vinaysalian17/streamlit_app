...
# Peak and Off PEak OCt to December
def is_peak_season(ds):
    date = pd.to_datetime(ds)
    return (date.month > 9 or date.month < 1)

    df_input['peak'] = df_input['ds'].apply(is_peak_season)
    df_input['off_peak'] = ~df_input['ds'].apply(is_peak_season)
...
if page == "Application":
    ...
    with st.container():
        ...
        with st.expander('Holidays'):
            ...
            countries = ['Country name','Australia','Italy','Spain','United States','France','Germany','Ukraine']
        ...
        with st.expander('Hyperparameters'):
            ...
            st.write("This is the proportion of the history in which the trend is allowed to change.")
            changepoint_range= st.select_slider(label= 'Changepoint range',options=changepoint_range_values)  
                ...
                with col1:
                    ...
                    if st.button('Export forecast (.csv)'):
                        with st.spinner("Exporting.."):
                            ...
                            export_forecast = pd.concat([pd21,pd11,pd22], axis=1, names=['ds','y','yhat'])

                            #st.write(export_forecast.head())
                            export_forecast = export_forecast.to_csv(decimal='.')

                            che = pd.concat([pd21,pd11,pd22], axis=1)

                            export_2 = che.resample('M',on='ds').sum()

                           # export_2 = pd.concat([pd211,pd111,pd212,pd112], axis=1, names=['ds','y','yhat','Month'])
                            #export_2 = export_forecast.resample('M').sum()
                            st.write(export_2.head(60))
                            export_2 = export_2.to_csv(decimal='.')
                            

                           # b64 = base64.b64encode(export_forecast.encode()).decode()
                           # href = f'<a href="data:file/csv;base64,{b64}">Download CSV File</a> (click  **forecast1.csv**)'
                           # st.markdown(href, unsafe_allow_html=True)


                            b65 = base64.b64encode(export_2.encode()).decode()
                            href = f'<a href="data:file/csv;base64,{b65}">Download CSV File</a> (click  **forecast.csv**)'
                            st.markdown(href, unsafe_allow_html=True)
